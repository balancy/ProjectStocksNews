...
def create_graph(tiker):
    ...
    """
    graph = Asset(tiker).get_hist_last_1_day()["Close"]
    ...
    try:
        graph_name = "flaskr/graphs/" + str(tiker) + ".png"
        plt.savefig(graph_name)
    except BaseException as ex:
        logger.exception(ex)
        graph_name = "Something wrong! Read utils.log"
    plt.clf()
    return graph_name


def create_candel_graph(tiker):
    """
    Создает последний дневной график с названием
    <Запрашиваемая_котировка>.png и сохраняет ее в основной каталог.
    График рисуется различным цветом в зависимости от текущей позиции за день.
